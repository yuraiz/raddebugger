...
os_file_iter_next(Arena *arena, OS_FileIter *iter, OS_FileInfo *info_out)
{
  ...
  {
    // rjf: get next entry
    mac_iter->dp = mac_iter->dir == 0 ? 0 : readdir(mac_iter->dir);
    good = (mac_iter->dp != 0);
  ...
  if(file_actions_init_code == 0)
  {
    ...
    if(attr_init_code == 0)
    {
      ...
      char **argv = push_array(scratch.arena, char *, params->cmd_line.node_count + 1);
      {
        ...
        U64 arg_idx = 1;
        for (String8Node *n = params->cmd_line.first->next;
          n != 0;
          n = n->next)
        {
      ...
      else
      {
        ...
        U64 env_idx = 0;
        for (String8Node *n = params->cmd_line.first;
          n != 0;
          n = n->next)
        {
...
main(int argc, char **argv)
{
  ...
  //- rjf: set up OS layer
  {
    ...
    // cache default environment
    {
      ...
      {
        default_env[idx] = (char *)push_str8_copy(os_mac_state.arena, str8_cstring(environ[idx])).str;
      }
