...
internal D_UnwindStepResult
ctrl_unwind_step__mac_arm64(D_Entity *module, D_Handle process_handle, REGS_RegBlockARM64 *regs, U64 endt_us)
{
  //////////////////////////////
      ...
      switch(process_entity->target_os)
      {
        ...
        case OperatingSystem_Mac:
        {
          switch(arch)
          {
            ...
            case Arch_arm64:
            {
              step_result = ctrl_unwind_step__mac_arm64(module_entity, process_entity->handle, regs_block, endt_us);
            }break;
          }
        }break;
      }
    ...
    if(got_event == 0) ProfScope("no event -> dmn_ctrl_run for a new one")
    {
      ...
      if(do_spoof) ProfScope("set spoof")
      {
        if(spoof_arch == Arch_arm64 && spoof->vaddr == 0)
        {
          ...
          reg_block.lr.u64 = spoof->new_ip_value;
          dmn_thread_write_reg_block(spoof->thread, &reg_block);
        }
        else
        {
