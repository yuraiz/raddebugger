      ...
      case RDI_EvalOp_TLSOff:
      {
        switch(e_interpret_ctx->tls_kind)
        {
          ...
          case E_TlsKind_MacOS:
          {
            ...
            B32 good_read = e_space_read(e_interpret_ctx->primary_space, &tlv_thunk, 0, r1u64(thunk_off, thunk_off+sizeof(tlv_thunk)));
            
            U64 base_vaddr = *e_interpret_ctx->tls_base & 0xfffffffffffffff8;

            U64 allocation_vaddr = 0;
            U64 alloc_off  = base_vaddr + (tlv_thunk.key << 3);
            good_read &= e_space_read(e_interpret_ctx->primary_space, &allocation_vaddr, 0, r1u64(alloc_off, alloc_off+sizeof(U64)));
