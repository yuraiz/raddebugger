...
r_metal_push_aligned( U64 size, U64 align )
{
  ...
  {
    wm_graphical_message(1, str8_lit("Fatal Allocation Failure"), str8_lit("Unexpected GPU memory allocation failure."));
    abort_self(1);
  }
  ...
  @autoreleasepool
  {
    ...
    //- brt: compile shaders
    {
      ...
      if (error)
      {
        ...
        wm_graphical_message(1, str8_lit("Fatal Error Compiling Shaders"), reason);
        abort_self(1);
      }
      ...
      {
        String8 reason = str8_cstring([[error localizedDescription] cStringUsingEncoding:NSUTF8StringEncoding]);
        wm_graphical_message(1, str8_lit("Error Creating Render Pipeline"), reason);
        abort_self(1);
      }
