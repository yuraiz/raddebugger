...
internal REGS_RegCode
reg_code_from_dw_reg_arm64(DW_Reg reg)
{
  switch (reg) {
#define X(_SRC, _SRC_ID, _DST_ID, ...) case _SRC_ID: return REGS_RegCodeARM64_##_DST_ID;
    DW_Regs_Arm64_XList
#undef X
  }
  return REGS_RegCodeARM64_NULL;
}
...
reg_code_from_dw_reg(Arch arch, DW_Reg reg)
{
  ...
  case Arch_arm64: { result = reg_code_from_dw_reg_arm64(reg); } break;
  case Arch_x86:
...
internal
MACHINE_OP_REG_READ(regs_read_dwarf_arm64)
{
  // map DWARF register to -> the internal register
  REGS_RegBlockARM64 *regs = ud;
  U64 reg_size = 0; void *reg_bytes = 0;
  switch (reg_id) {
#define X(_N, _ID, _MAP_N, ...) case _ID: { reg_size = sizeof(regs->_MAP_N); reg_bytes = &regs->_MAP_N; } break;
    DW_Regs_Arm64_XList
...
internal
MACHINE_OP_REG_WRITE(regs_write_dwarf_arm64)
{
  // map DWARF register to -> the internal register
  REGS_RegBlockARM64 *regs = ud;
  U64 reg_size = 0; void *reg_bytes = 0;
