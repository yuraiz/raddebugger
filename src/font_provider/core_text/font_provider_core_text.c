...
internal void
fp_ct_apply_raster_flags(CGContextRef ctx, FP_RasterFlags flags)
{
  B32 smooth = !!(flags & FP_RasterFlag_Smooth);
  //CGContextSetShouldAntialias(ctx, smooth ? true : false);
  if (@available(macOS 10.6, *))
  {
    //CGContextSetAllowsAntialiasing(ctx, smooth ? true : false);
...
fp_metrics_from_font(FP_Handle handle)
{
  ...
  if (font.ct_font != 0)
  {
    ...
    // CoreGraphics does not expose cap height on CGFont directly.
    // Estimate from a temporary CTFont sized to unitsPerEm so values land in design-space.
    if (units_per_em > 0)
    {
...
fp_raster(Arena *arena, FP_Handle font_handle, F32 size, FP_RasterFlags flags, String8 string)
{
  ...
  // Simple one-codepoint-per-glyph mapping, mirroring original backend.
  {
    F32 pen_x = 0.f;
  ...
  // Match the original's 8-byte-ish width padding.
  atlas_w += 7;
  atlas_w -= atlas_w % 8;
  ...
  if (ctx != 0)
  {
    ...
    fp_ct_apply_raster_flags(ctx, flags);

    CGContextSetRGBFillColor(ctx, 1.f, 1.f, 1.f, 1.f);

    // In CoreGraphics' default space, origin is bottom-left.
    // So place the baseline descent pixels above the bottom.
    ...
    // Normalize output so RGB is white and alpha carries coverage,
    // matching the original backend contract.
    {
      U64 color_sum = 0;
